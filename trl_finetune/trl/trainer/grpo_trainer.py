...
class RepeatSampler(Sampler):
    """
    데이터셋의 인덱스를 구조화된 방식으로 반복하는 샘플러.
    
    GRPO에서 각 배치 내에서 같은 데이터를 여러 번 사용해 다양한 응답을 생성하고
    그룹 상대 보상을 계산하는 데 사용됨.
...
# torch.nanstd는 없으므로 여기서 정의
def nanstd(tensor: torch.Tensor) -> torch.Tensor:
    """
    NaN을 무시하고 텐서의 표준 편차를 계산. 이 함수는 1D 텐서만 지원함.
    ...
    Returns:
        `torch.Tensor`:
            NaN을 무시한 텐서의 표준 편차.
    """
    variance = torch.nanmean((tensor - torch.nanmean(tensor, keepdim=True)) ** 2)  # NaN을 무시하고 분산 계산
    count = torch.sum(~torch.isnan(tensor))  # NaN이 아닌 값의 개수
    variance *= count / (count - 1)  # 베셀 보정
...
def split_tensor_dict(
    ...
    """
    텐서 딕셔너리를 첫 번째 차원을 따라 `num_chunks`개의 동일한 부분으로 분할.
...
def shuffle_sequence_dict(seq_dict: dict[str, Optional[Sequence]]) -> dict[str, Optional[Sequence]]:
    """
    딕셔너리 내 모든 시퀀스형 값들을 첫 번째 차원을 따라 동일하게 섞음.
...
def nanmin(tensor: torch.Tensor) -> torch.Tensor:
    """
    NaN을 무시하고 텐서의 최솟값을 계산. 이 함수는 1D 텐서만 지원함.

    Args:
        tensor (`torch.Tensor`): 입력 텐서, 모양 `(N,)`.

    Returns:
        `torch.Tensor`: NaN을 무시한 텐서의 최솟값. 모든 값이 NaN이면 NaN 반환.
    """
...
def nanmax(tensor: torch.Tensor) -> torch.Tensor:
    """
    NaN을 무시하고 텐서의 최댓값을 계산. 이 함수는 1D 텐서만 지원함.

    Args:
        tensor (`torch.Tensor`): 입력 텐서, 모양 `(N,)`.

    Returns:
        `torch.Tensor`: NaN을 무시한 텐서의 최댓값. 모든 값이 NaN이면 NaN 반환.
    """
    if torch.isnan(tensor).all():
        return torch.tensor(float("nan"), dtype=tensor.dtype, device=tensor.device)
...
def split_pixel_values_by_grid(batch: dict[str, torch.Tensor]) -> dict[str, Union[torch.Tensor, list[torch.Tensor]]]:
    """
    `batch["image_grid_thw"]` 각 행의 곱을 기반으로 `batch["pixel_values"]`를 텐서 리스트로 분할.
    다른 항목들은 변경하지 않음.
    """
    ...
    pixel_values = batch["pixel_values"]  # [전체, 특징_차원]
...
def unsplit_pixel_values_by_grid(batch: dict[str, Union[torch.Tensor, list[torch.Tensor]]]) -> dict[str, torch.Tensor]:
    """
    `split_pixel_values_by_grid`의 반대 작업. `batch["pixel_values"]`의 텐서 리스트를
    첫 번째 차원을 따라 하나의 텐서로 다시 병합.
    """
...
def truncate_with_protected_tokens(
    ...
    """
    보호된 토큰들을 유지하면서 텐서를 목표 길이로 자름.

    Args:
        ids (`torch.Tensor`):
            토큰 ID의 입력 텐서, 모양 (batch_size, sequence_length).
        mask (`torch.Tensor`):
            어텐션 마스크의 입력 텐서, 모양 (batch_size, sequence_length).
        target_length (`int`):
            출력 시퀀스의 원하는 길이.
        protected_tokens (`list[int]`):
            출력에서 보존되어야 하는 토큰 ID 리스트.
    """
...
class GRPOTrainer(Trainer): #Trainer에 백워드 포함 (상속받음)
    """
    Group Relative Policy Optimization (GRPO) 방법을 위한 트레이너.
    이 알고리즘은 [DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language
    Models](https://huggingface.co/papers/2402.03300) 논문에서 처음 제안됨.
    ...
    인자:
        model (`Union[str, PreTrainedModel]`):
            학습할 모델. 다음 중 하나일 수 있음:

            - 문자열: huggingface.co에 호스팅된 사전학습 모델의 *모델 ID*, 또는
              [`~transformers.PreTrainedModel.save_pretrained`]로 저장된 모델 가중치를 포함하는
              *디렉토리* 경로, 예: `'./my_model_directory/'`. 모델은
              `args.model_init_kwargs`의 키워드 인자와 함께
              [`~transformers.AutoModelForCausalLM.from_pretrained`]를 사용하여 로드됨.
            - [`~transformers.PreTrainedModel`] 객체. 인과적 언어 모델만 지원됨.
        reward_funcs (`Union[RewardFunc, list[RewardFunc]]`):
            보상을 계산하는 데 사용될 보상 함수. 보상을 계산하기 위해 모든 보상 함수를
            프롬프트와 완성된 텍스트로 호출하고 보상을 합산함. 다음 중 하나일 수 있음:

            - 단일 보상 함수:
                - 문자열: huggingface.co에 호스팅된 사전학습 모델의 *모델 ID*, 또는
                [`~transformers.PreTrainedModel.save_pretrained`]로 저장된 모델 가중치를 포함하는
                *디렉토리* 경로, 예: `'./my_model_directory/'`. 모델은 `num_labels=1`과
                `args.model_init_kwargs`의 키워드 인자와 함께
                [`~transformers.AutoModelForSequenceClassification.from_pretrained`]를 사용하여 로드됨.
                - [`~transformers.PreTrainedModel`] 객체: 시퀀스 분류 모델만 지원됨.
                - 커스텀 보상 함수: 함수는 프롬프트와 생성된 완성 텍스트, 그리고 데이터셋의
                  추가 열들을 입력받음. 보상 리스트를 반환해야 함. 커스텀 보상 함수는
                  해당 샘플에 보상이 적용되지 않을 때 `None`을 반환할 수도 있음.
                  이는 다른 유형의 샘플에 다른 보상 함수가 적용되는 멀티태스크 학습에 유용함.
                  보상 함수가 샘플에 대해 `None`을 반환하면, 해당 샘플의 보상 계산에서
                  그 보상 함수는 제외됨. 자세한 내용은 [커스텀 보상 함수 사용하기](#using-a-custom-reward-function) 참조.

                  트레이너의 상태도 보상 함수에 전달됨. 트레이너 상태는
                  [`~transformers.TrainerState`]의 인스턴스이며 보상 함수 시그니처의
                  `trainer_state` 인자로 접근 가능.
            - 보상 함수 리스트: 각 항목은 위의 유형 중 하나일 수 있음.
            리스트 내에서 다른 유형을 혼합하는 것(예: 문자열 모델 ID와 커스텀 보상 함수)이 허용됨.
        args ([`GRPOConfig`], *선택사항*, 기본값 `None`):
            이 트레이너의 설정. `None`인 경우 기본 설정이 사용됨.
        train_dataset ([`~datasets.Dataset`] 또는 [`~datasets.IterableDataset`]):
            학습에 사용할 데이터셋. `"prompt"` 열을 포함해야 함. 데이터셋의 추가 열은
            무시됨. 샘플 형식은 다음 중 하나일 수 있음:

            - [표준](dataset_formats#standard): 각 샘플은 일반 텍스트를 포함.
            - [대화형](dataset_formats#conversational): 각 샘플은 구조화된 메시지(예: 역할과
              내용)를 포함.
        eval_dataset ([`~datasets.Dataset`], [`~datasets.IterableDataset`] 또는 `dict[str, Union[Dataset,
        IterableDataset]]`):
            평가에 사용할 데이터셋. `train_dataset`과 동일한 요구사항을 충족해야 함.
        processing_class ([`~transformers.PreTrainedTokenizerBase`], [`~transformers.ProcessorMixin`] 또는 `None`, *선택사항*, 기본값 `None`):
            데이터 처리에 사용되는 처리 클래스. 패딩 방향은 "left"로 설정되어야 함. `None`인 경우
            모델 이름에서 [`~transformers.AutoProcessor.from_pretrained`]로 처리 클래스가 로드됨.
            패딩 토큰 `tokenizer.pad_token`이 설정되어야 함. 처리 클래스에 패딩 토큰이 설정되지 않은 경우
            `tokenizer.eos_token`이 기본값으로 사용됨.
        reward_processing_classes (`Union[PreTrainedTokenizerBase, list[PreTrainedTokenizerBase]]`, *선택사항*, 기본값 `None`):
            `reward_funcs`에 지정된 보상 함수에 해당하는 처리 클래스. 다음 중 하나일 수 있음:

            - 단일 처리 클래스: `reward_funcs`에 보상 함수가 하나만 있을 때 사용.
            - 처리 클래스 리스트: `reward_funcs`의 보상 함수 순서와 길이가 일치해야 함.
            `None`으로 설정하거나 [`~transformers.PreTrainedModel`]에 해당하는 리스트 요소가
            `None`인 경우, 모델의 토크나이저가 [`~transformers.AutoTokenizer.from_pretrained`]를
            사용하여 자동으로 로드됨. `reward_funcs`의 커스텀 보상 함수(즉, [`~transformers.PreTrainedModel`]이
            아닌 것)에 해당하는 `reward_processing_classes`의 항목은 무시됨.
        callbacks ([`~transformers.TrainerCallback`] 리스트, *선택사항*, 기본값 `None`):
            학습 루프를 커스터마이즈하기 위한 콜백 리스트. [여기](https://huggingface.co/docs/transformers/main_classes/callback)에
            설명된 기본 콜백 리스트에 추가됨.

            기본 콜백 중 하나를 제거하려면 [`~transformers.Trainer.remove_callback`]
            메서드를 사용.
        optimizers (`tuple[torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR]`, *선택사항*, 기본값 `(None,
        None)`):
            사용할 옵티마이저와 스케줄러를 포함하는 튜플. 기본값은 모델에 대한 [`AdamW`] 인스턴스와
            `args`로 제어되는 [`get_linear_schedule_with_warmup`]에서 제공하는 스케줄러.
        peft_config ([`~peft.PeftConfig`], *선택사항*, 기본값 `None`):
            모델을 래핑하는 데 사용되는 PEFT 설정. `None`인 경우 모델이 래핑되지 않음.
    """
    ...
    def __init__(
        ...
        reward_funcs: Union[RewardFunc, list[RewardFunc]], # 리워드(밖에서 정의)
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                pass  # torch_dtype은 이미 torch.dtype 또는 "auto" 또는 None
            elif isinstance(torch_dtype, str):  # 문자열이지만 "auto"가 아님
                torch_dtype = getattr(torch, torch_dtype)
                model_init_kwargs["torch_dtype"] = torch_dtype
            else:
                raise ValueError(
                    "`GRPOConfig`에 잘못된 `torch_dtype`이 전달됨. 'auto' 또는 `torch.dtype`을 나타내는 "
                    f"문자열(예: 'float32')이 필요하지만 {torch_dtype}이(가) 전달됨."
                )
            # gradient checkpointing이 활성화된 경우 캐싱 비활성화 (지원되지 않음)
            ...
            if args.model_init_kwargs is not None:
                warnings.warn(
                    "`GRPOConfig`에 `model_init_kwargs`를 전달했지만 모델이 이미 인스턴스화됨. "
                    "`model_init_kwargs`는 무시됨."
                )
        ...
        if isinstance(processing_class, ProcessorMixin):
            ...
            raise TypeError("`processing_class`는 `PreTrainedTokenizerBase` 또는 `ProcessorMixin`이어야 함")
        ...
        if self.use_liger_loss and self.top_entropy_quantile < 1.0:
            raise NotImplementedError(
                "Liger 커널은 현재 엔트로피 기반 토큰 위치 마스킹을 지원하지 않음."
            )
        if self.use_liger_loss and not self.importance_sampling_level == "token":
            raise NotImplementedError(
                "Liger 커널은 현재 토큰 수준 중요도 샘플링만 지원함. "
                "`importance_sampling_level`을 'token'으로 설정해 주세요."
            )
        ...
        if (
            ...
            # https://github.com/huggingface/trl/issues/3213 참조
            raise NotImplementedError(
                "Iterable 데이터셋은 아직 GRPOTrainer에서 지원되지 않음. 표준 데이터셋을 사용해 주세요."
            )
        ...
        super().__init__(
            model=model,
            args=args,
            data_collator=identity,  # GRPO에서는 데이터 콜레이션이 필요하지 않음
        ...
        if self.use_liger_loss:
            if not is_liger_kernel_available():
                raise ImportError(
                    "GRPO loss로 `liger_loss`를 사용하려면 Liger가 필요함. `pip install liger-kernel` 실행."
                )
        ...
        if self.use_vllm:
            if not is_vllm_available():
                raise ImportError(
                    "vLLM을 사용할 수 없고 `use_vllm`이 True로 설정됨. "
                    "`pip install vllm`으로 vLLM을 설치해 주세요."
                )
            ...
            if self.vllm_mode == "server":
                ...
                raise ValueError(f"vllm_mode는 'server' 또는 'colocate'여야 하는데 '{self.vllm_mode}'이(가) 전달됨.")
